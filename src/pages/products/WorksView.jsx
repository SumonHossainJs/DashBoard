...
const WorksView = () => {
  ...
  const handleDelete = async (id, refreshData) => {
    try {
      const res = await newRequest.delete(`/workItem/delete/${id}`);
  ...
  return (
    <div className="products">
      ...
      <div className="info">
        <h2> {type} WorksView</h2>
        <Link to={"/upload-works"}>
          <button onClick={() => setOpen(true)}>Add New WorksView</button>
        </Link>
      </div>
